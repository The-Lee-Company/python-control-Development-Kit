...
if __name__ == '__main__':
    """"
    Sets the pump to track a target flow using an external flow sensor - SENSIRION SLF3S-1300F. 
    The drive power and flow are printed out for 10s.
    Flow is only available with the GP driver.
    """

    # create a Disc Pump instance
    myPump = LVDiscPump()

    # A GP driver always uses UART.
    ...
    # If you want to PID control to negative flow, the PID terms and the SetVal flow target need to be negative
    # set the pump to PID flow control with target pressure to the SetVal register
    myPump.set_pid_flow_control_with_set_val(p_term=10, i_term=50)

    # set the flow measurement units to ml/min (other units can equally be used)
    myPump.write_reg(LVRegister.FLOW_MEAS_UNIT, LVMeasUnits.FLOW_mL_PER_MIN)

    # set the target pressure to 12 ml/min
    # Note that the pump power cannot exceed 1000mW by default
    myPump.write_reg(LVRegister.SET_VAL, 12)
    ...
    while current_time - start_time < 30:
        # read the drive power and flow of the pump and print them every 0.5s
        drive_power = myPump.read_register(LVRegister.MEAS_DRIVE_MILLIWATTS)
        flow = myPump.read_register(LVRegister.MEAS_FLOW)
        current_time = float(time.time())
        print(f'Time [s] {current_time - start_time:.1f}, Drive power [mW] {drive_power:.1f}, Flow [ml/min] {flow:.1f}')
        time.sleep(0.5)
